...
public abstract class InGameHudMixin {
    ...
    private void renderHealthBarMixin(MatrixStack matrices, PlayerEntity player, int x, int y, int lines, int regeneratingHeartIndex, float maxHealth, int lastHealth, int health, int absorption, boolean blinking, CallbackInfo ci){
        if (SynchroClient.applyNewHud) {
            ...
            int color = NewHudUtil.Colors.HEART_TYPE_COLOR_MAP.get(heartType).color;
            ...
            x += OX + 1;
            InGameHud.fill(matrices, x, y, x+L_OUT, y+H_OUT, NewHudUtil.Colors.HEALTH_BAR_OUTLINE.color);
            x += 1;
            y += 1;
            if (damaging) InGameHud.fill(matrices, x, y, x+lastHealthLength, y+H_BAR, NewHudUtil.Colors.HEALTH_BAR_DAMAGING.color);
            InGameHud.fill(matrices, x, y, healthBarLength > 0 ? x + healthBarLength : x, y+H_BAR, color);
            haveAbsorption = absorptionLength > 0;
            InGameHud.fill(matrices, x, y, haveAbsorption ? absorptionLength < L_BAR ? x+absorptionLength : x+L_BAR : x, y+H_BAR, NewHudUtil.Colors.HEALTH_BAR_ABSORBING.color);
    ...
    private void drawNewHungerBar(MatrixStack matrices, CallbackInfo ci,
        ...
        if (SynchroClient.applyNewHud){
            ...
            InGameHud.fill(matrices, x-L_OUT, y, x, y+H_OUT, NewHudUtil.Colors.HUNGER_BAR_OUTLINE.color);
            x -= 1;
            y += 1;
            InGameHud.fill(matrices, x-foodLength, y, x, y+H_BAR, playerEntity.hasStatusEffect(StatusEffects.HUNGER) ? NewHudUtil.Colors.HUNGER_BAR_HUNGER.color : NewHudUtil.Colors.HUNGER_BAR_NORMAL.color);
            if (saturationLength == 0){
                InGameHud.fill(matrices, x-foodLength, y, x-foodLength+exhaustionLength, y+H_BAR, NewHudUtil.Colors.HUNGER_BAR_EXHAUSTION.color);
                haveSaturation = false;
            }
            else {
                InGameHud.fill(matrices, x-saturationLength, y, x, y+H_BAR, NewHudUtil.Colors.HUNGER_BAR_SATURATION.color);
                InGameHud.fill(matrices, x-saturationLength, y, x-saturationLength+exhaustionLength, y+H_BAR, NewHudUtil.Colors.HUNGER_BAR_EXHAUSTION.color);
            }
    ...
    private void drawNewAirBar(MatrixStack matrices, CallbackInfo ci,
        ...
        if (SynchroClient.applyNewHud){
            ...
            InGameHud.fill(matrices, x-L_OUT, y, x, y+H_OUT, NewHudUtil.Colors.AIR_BAR_OUTLINE.color);
            if (air < 0) InGameHud.fill(matrices, x-L_OUT, y, x-L_OUT+Math.abs(air*L_OUT/20), y+H_OUT, NewHudUtil.Colors.AIR_BAR_DROWNING.color);
            x -= 1;
            y += 1;
            InGameHud.fill(matrices, x-L_BAR, y, x-L_BAR+Math.min(filledLength, L_BAR), y+H_BAR, NewHudUtil.Colors.AIR_BAR_NORMAL.color);
    ...
    private void endRenderingStatusBars(MatrixStack matrices, CallbackInfo ci){
        if (SynchroClient.applyNewHud){
            ...
            int fireTicks;
            if (player instanceof PlayerFireTickSync playerEntity) fireTicks = playerEntity.synchro$getTheFireTicks();
            else fireTicks = -20;
    ...
    private void drawGasOverlay(MatrixStack matrices, float tickDelta){
        ...
        int gasSubmersionColor = CamaraUtil.getGasOverlayColor(this.client.world, camera);
