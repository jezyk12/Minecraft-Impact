package name.synchro.fluidRender;
...
public class TranslucentGasRenderer implements FluidRenderHandler {
    public static final Identifier GAS_TEXTURE_ID = Identifier.of(Synchro.MOD_ID, "block/gas");
    ...
    protected void vertex(VertexConsumer vertexConsumer, double[] xyz0, double[] xyz, float[] rgba, float[] uv, int light) {
        ...
        vertexConsumer.vertex((float) x, (float) y, (float) z).color(rgba[0], rgba[1], rgba[2], rgba[3]).texture(uv[0], uv[1]).light(light).normal(0.0F, 1.0F, 0.0F);
    }
