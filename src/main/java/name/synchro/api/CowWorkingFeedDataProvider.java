...
public abstract class CowWorkingFeedDataProvider extends FabricCodecDataProvider<Set<CowWorkingFeedsData.Entry>> {
    ...
    private final Set<CowWorkingFeedsData.Entry> entries = new HashSet<>();
    ...
    @Override
    protected void configure(BiConsumer<Identifier, Set<CowWorkingFeedsData.Entry>> provider, RegistryWrapper.WrapperLookup lookup) {
        generate(lookup);
    ...
    public void addEntry(Ingredient ingredient, int time){
        this.entries.add(new CowWorkingFeedsData.Entry(ingredient, time));
    }
