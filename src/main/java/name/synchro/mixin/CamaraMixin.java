...
import name.synchro.synchroBlocks.AbstractGas;
import name.synchro.synchroBlocks.GasBlock;
...
@Mixin(Camera.class)
public class CamaraMixin implements CameraDuck {
    @Shadow
    ...
    public CameraGasSubmersionType getGasSubmersionType(){
        ...
        if (fluidState.getFluid() instanceof AbstractGas && this.pos.y < (double)(this.blockPos.getY() + 1)) {
            int level = fluidState.getLevel();
            return CameraGasSubmersionType.values()[(level + 1) / 2];
        }
        return CameraGasSubmersionType.NONE;
    }
}
